...
2. Điền thông tin vào biểu mẫu:
   ...
   * **Package Name**: Điền đúng tên khai báo trong file `package.json` của bạn (ở đây là **`com.gemini.swagger-dto-generator`**).
   * **Category**: Chọn danh mục phù hợp (ví dụ: `utility` hoặc `editor-tool`).
...
2. Thêm một Registry mới:
   ...
   * **Scope(s)**: Thêm scope **`com.gemini.swagger-dto-generator`**
3. Bấm **Apply**.
...
```bash
openupm add com.gemini.swagger-dto-generator
```
